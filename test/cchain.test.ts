...
async function signAndSendCChainTx(rawTx: any) {
  ...
  const resolution = await ledgerService.resolveTransaction(unPrefix0x(tx.unsignedSerialized), {}, {
    erc20: true,
    externalPlugins: true
  })
  const signature = await flare.signEVMTransaction(CCHAIN_PATH, unPrefix0x(tx.unsignedSerialized), resolution)
...
async function contractCallTx(abi: any, address: string, method: string, params: any[], value: string | number): Promise<any> {
  ...
  const callTx = await contract.getFunction(method).populateTransaction(...params)
  return {
    ...callTx,
...
describe('Zondax evm methods', () => {
  ...
  it.only("should sign and send an erc20 transfer transaction", async () => {
